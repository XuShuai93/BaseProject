...
//                    val options = decorView.systemUiVisibility or View.SYSTEM_UI_FLAG_LAYOUT_STABLE or
...
//                    decorView.systemUiVisibility = options
        ...
        @JvmStatic
        fun restoreStatusBar(window: Window?) {
            window?.apply {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
//                    //取消状态栏透明
//                    clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS)
...
//                    decorView.systemUiVisibility = options
         ...
         */
        @JvmStatic
        fun setStatusBarLightFont(activity: Activity, lightFont: Boolean) {
            activity.window?.apply {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                    insetsController?.let {
                        WindowInsetsControllerCompat.toWindowInsetsControllerCompat(it).isAppearanceLightStatusBars =
                            lightFont
                    }
         ...
         */
        @JvmStatic
        fun isStatusBarLightFont(activity: Activity): Boolean {
            return activity.window?.let {
                // 设置浅色状态栏时的界面显示
                return when {
                    Build.VERSION.SDK_INT >= Build.VERSION_CODES.R -> {
                        it.insetsController?.let { controller ->
                            WindowInsetsControllerCompat.toWindowInsetsControllerCompat(controller).isAppearanceLightStatusBars
                        } ?: false
                    }
